...
def generate_forecast_html(storms: list[dict], output_path: str, frames_dir: str = None,
    ...
    cards_html = []
    track_ids = [storm.get('track_id', 'Unknown') for storm in storms if isinstance(storm, dict)]
    ...
    for storm in storms:
        ...
        jtwc_data = current_info.get('jtwc', {})
        ...
        animation_html = ""
        if storm_frames_dir and os.path.exists(storm_frames_dir):
            ...
            if frame_files:
                ...
                animation_html = f"""
            <div class="panel" id="anim-section-{track_id}">
                <div class="panel-header">
                    <span class="panel-icon">🎬</span>
                    <span>Track Evolution Animation</span>
                </div>
                <div class="animation-player">
                    <div class="frame-wrapper" onclick="openLightbox(document.getElementById('animation-frame-{track_id}').src)">
                        <img id="animation-frame-{track_id}"
                    ...
                    <div class="player-controls">
                        <div class="controls-row">
                            <button class="control-btn" onclick="playAnimation('{track_id}')" id="btn-play-{track_id}">▶ Play</button>
                            <button class="control-btn secondary" onclick="pauseAnimation('{track_id}')">⏸ Pause</button>
                            <button class="control-btn secondary" onclick="resetAnimation('{track_id}')">⟲ Reset</button>
                            <div class="progress-bar-container">
                                <input type="range" id="progress-{track_id}" class="progress-slider"
                                       min="0" max="{len(frame_files)-1}" value="0"
                                       oninput="seekFrame('{track_id}', this.value)">
                                <span id="frame-counter-{track_id}" class="frame-counter">1/{len(frame_files)}</span>
                            </div>
                        </div>
                        <div class="controls-row speed-row">
                            <span class="speed-label">⚡ Speed:</span>
                            <input type="range" id="speed-{track_id}" class="speed-slider"
                                   min="1" max="8" value="2" step="0.5"
                                   oninput="updateSpeed('{track_id}', this.value)">
                            <span id="speed-val-{track_id}" class="speed-value">2×</span>
        ...
        if animation_html:
            cards_html.append(animation_html)
        ...
        cards_html.append(f"""
            <div class="maps-grid">
                <div class="panel">
                    ...
                    <p class="map-note">
                        Gray = ensemble members &nbsp;·&nbsp; Red = mean track &nbsp;·&nbsp;
                ...
                <div class="genesis-body">
                    <div class="map-image-wrapper genesis-map-wrapper"
                        ...
                        <img src="{genesis_img_name}"
                             ...
                             onerror="this.closest('.genesis-panel').style.display='none'">
                        <div class="zoom-hint">🔍 Click to enlarge</div>
                    </div>
                    <div class="genesis-legend">
                        <div class="genesis-legend-title">MSLP Color Scale</div>
                        <div class="genesis-legend-items">
                            <span class="g-chip" style="background:#CC0000;color:#fff;">≤935 hPa</span>
                            <span class="g-chip" style="background:#FF6600;color:#fff;">936–955 hPa</span>
                            <span class="g-chip" style="background:#FFB800;color:#000;">956–978 hPa</span>
                            <span class="g-chip" style="background:#FFD966;color:#000;">979–988 hPa</span>
                            <span class="g-chip" style="background:#4488FF;color:#fff;">989–1000 hPa</span>
                            <span class="g-chip" style="background:#AACCFF;color:#000;">&gt;1000 hPa</span>
                        </div>
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after {{ margin:0; padding:0; box-sizing:border-box; }}
        ...
        .radii-value {{ display: flex; flex-wrap: wrap; gap: 7px; margin-top: 4px; }}
        .radii-quad {{
            background: var(--surface-3); padding: 3px 9px;
            border-radius: 6px; font-size: .83em;
        ...
        @media (max-width: 600px) {{
            ...
            .radii-value {{ display: grid; grid-template-columns: 1fr 1fr; width: 100%; }}
        ...
        @media (max-width: 380px) {{
            ...
            .radii-value {{ grid-template-columns: 1fr; }}
        }}
...
{cards_section}
    ...
    document.addEventListener('keydown', e => {{
        ...
        const trackIds = Object.keys(animationState);
        if (!trackIds.length) return;
        const tid = trackIds[0];
        if (e.code === 'Space') {{
            e.preventDefault();
